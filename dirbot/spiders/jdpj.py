...
# local rules
from dirbot.spiders.model.jd_com.peiJian.rules import JDPJ_RULES
from dirbot.items import Website

class JDPJSpider(Spider):
    rules = JDPJ_RULES()
    name = rules.name
    ...
    def parse(self, response):
        ...
        for body in bodies:
            ...
            # jd_root_nav
            item['jfpj_root_nav'] = body.xpath(self.rules.filters['root_nav']['0']).extract()
            ...
            item['jfpj_spec_n1'] = body.xpath(self.rules.filters['spec_n1']['0']).extract()
            ## this rules'jd_p_ad' is right, but cannot select. I don't know why
            item['jfpj_p_ad'] = body.xpath(self.rules.filters['p_ad']['0']).extract()

            ## this rules'jd_jd_price' is right, but cannot select. I don't know why
            item['jfpj_jd_price'] = body.xpath(self.rules.filters['jd_price']['0']).extract()
            ...
            item['jfpj_canshu'] = body.xpath(self.rules.filters['canshu']['0']).extract()
